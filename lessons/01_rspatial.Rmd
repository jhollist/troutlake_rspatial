...
Also, there are some really nice resource available for learning how to use R for spatial data analysis.  A couple I like:
...
On Windows, getting `sf` added is no different than adding any other package that is on CRAN.  If you are on linux or macOS then you'll need to put in a bit more effort (i.e. need to have GDAL installed).  Best instructions for getting all this set up are at <https://github.com/r-spatial/sf#installing>.  If you are on macOS, you will need XCode and  `homebrew` installed.  To do so, open up a Terminal window the type:
...
library("sp")
```
If you've succesfully installed `sf` (and gdal and geos) on macOS then you should be good to go with the above functions for installing `rgdal` and `sp` on macOS.
